...
const f1 = () => {
    let out = a1.every(item => item > a1_from && item < a1_to);
...
const f2 = () => {
    let out = a2.every(item => item > a2_from && item < a2_to);
...
const f5 = () => {
    let adultsRes = a5
        ...
        .every(item => item['pay'] === true);
    document.querySelector('.out-5').textContent = adultsRes;
}
...
const f6 = () => {
    ...
    let out = counter === a6.length ? true : false;
...
const f9 = () => {
    let out = a9.some(item => item.age <= 16);
